...
class MilvusUserDB:
    def __init__(
        ...
        port=DB_PORT,
        db_name=DB_NAME,
        user_collection_name=DB_USER_COLLECTION,
    ):
    ...
    def setup_collection(self):
        if not utility.has_collection(self.collection_name):
            user_fields = [
                ...
                FieldSchema(
                    name="password_hash", dtype=DataType.VARCHAR, max_length=128
                ),
                FieldSchema(name="user_vector", dtype=DataType.FLOAT_VECTOR, dim=2),
            ]
            user_schema = CollectionSchema(
                fields=user_fields, description="User collection"
            )
            self.user_collection = Collection(
                self.collection_name, user_schema, consistency_level="Strong"
            )
